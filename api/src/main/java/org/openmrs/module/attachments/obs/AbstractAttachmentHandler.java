...
public abstract class AbstractAttachmentHandler implements ComplexObsHandler {
	...
	*/	
	...
	/**
	* <p>The saveThumbnailOrRename method checks image dimension to see if the image
	* is small enough to be its own thumbnail.  If so, it will rename the image file
	* by appending the <b>NO_THUMBNAIL_SUFFIX</b> to the file.  Otherewise, it will create
	* a small thumbnail file alongside the original file to be used as thumbnail image.</p>
	*
	...
	*/
	public static String saveThumbnailOrRename(File savedFile, int imageHight, int imageWidth){

       String savedFileName = savedFile.getName();

       // Check for small image file
       if ((imageHight <= THUMBNAIL_MAX_HEIGHT) && (imageWidth <= THUMBNAIL_MAX_WIDTH)){
